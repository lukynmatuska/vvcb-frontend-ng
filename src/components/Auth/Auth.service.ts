...
export class Auth {
    ...
    public async buildUserAsync(): Promise<User>{
        await this.check();
        let parsed = this.keycloak.tokenParsed;
        if(!parsed){
            return null;
        ...
        return new Promise((resolve) => {
            resolve(new User(parsed["sub"], parsed["preferred_username"], parsed["given_name"], parsed["family_name"], parsed["realm_access"]["roles"]));
        });
    }
    ...
    private async init(initParams: {}) {
        ...
        .then((authenticated) => {
            ...
            this.keycloak.updateToken(3).then((refreshed) => {
                if (refreshed) {
                    localStorage.setItem(
                        this.localStorageMapping.access_token,
                        this.keycloak.token
    ...
    public async refresh() {
        await this.init(this.buildInitParams());
    }
